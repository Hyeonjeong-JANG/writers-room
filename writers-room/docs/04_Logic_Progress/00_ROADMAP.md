...
- [x] API: GET /api/dashboard/stats (통합 통계)
- [x] 대시보드 메인 (통계 카드: 스토리, 에이전트, 수익, 기여)
- [x] 내 스토리 관리 (상태 변경, 작가방 바로가기)
- [x] 내 에이전트 관리 (수정, 비활성화, 수익 확인)
- [x] 수익 현황 페이지 (차트, 거래 내역, Explorer 링크)
- [x] 기여 히스토리 페이지
- [x] 프로필 페이지 (/profile/[address])
